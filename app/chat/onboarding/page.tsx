...
export default function ChatPage() {
  ...
  const [showTutorial1, setShowTutorial1] = useState(false); // "View cheat code" tutorial
  ...
  // Handle onboarding chat data from sessionStorage - MUST RUN FIRST
  useEffect(function loadOnboardingData() {
    // Only run once
    if (onboardingLoadedRef.current) {
      return;
    }

    const onboardingData = sessionStorage.getItem('onboardingChat');
    ...
    try {
      ...
      const { userAnswer, coachMessage, isOnboarding } = parsed;
      ...
      // Add user message (their zone state answer)
      const userMsg: Message = {
        id: uid(),
        text: userAnswer,
        sender: 'user',
        timestamp: new Date(),
        isHistoric: true // No typing animation
      };

      // Add coach message with the cheat code
      ...
      messageIds.current.add(userMsg.id);
      messageIds.current.add(coachMsg.id);
      ...
      // Prevent welcome message from showing
      pendingWelcome.current = true;

      // Show first tutorial after brief delay
      setTimeout(() => {
        setShowTutorial1(true);
      }, 1500);

      // Clear the onboarding data
      sessionStorage.removeItem('onboardingChat');
    } catch (err) {
      console.error('Error loading onboarding chat:', err);
    }
  });
  ...
  const handleBack = () => {
    // In onboarding mode, go back to the onboarding flow
    if (isOnboardingMode) {
  ...
  useEffect(() => {
    ...
    const initializeChat = async () => {
      // FIRST: Check for onboarding data - this takes priority
      const onboardingData = sessionStorage.getItem('onboardingChat');
      if (onboardingData) {
        console.log('[Init] Found onboarding data, skipping normal initialization');
        return; // Let the onboarding effect handle it
      }
...
**Practice:** 20 free throws daily using this exact sequence`,
  ...
  return (
    <div className="fixed inset-0 font-sans overflow-hidden" style={{ color: 'var(--text-primary)' }}>
                          ...
                          return (
                            <div className="w-full">
                              ...
                              <div className="flex justify-center w-full px-2">
                                <button
                                  onClick={() => {
                                    setSelectedCheatCode({ ...cheatCode, messageId: message.id, messageText: cheatCodeText });
                                    // Dismiss tutorial 1 and show tutorial 2 for onboarding
                                    if (isOnboardingMode && showTutorial1) {
      ...
      {/* Tutorial 1: Point to "View Cheat Code" button */}
      {showTutorial1 && isOnboardingMode && (
        <>
          <div className="fixed inset-0 bg-black/60 z-[60]" onClick={() => setShowTutorial1(false)} />
          <div
